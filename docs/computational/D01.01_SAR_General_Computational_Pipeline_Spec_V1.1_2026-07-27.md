...
O1 Logging, Replay, and Evaluation
     | observes L0, L1, L2, L3, L4A, L4B, L5A, and L5B
...
- **L0 Edge Capture and Transport** performs source-adjacent acquisition on constrained or distributed nodes (for example, ESP32 + ADC), frames raw capture packets, and transmits them to ingest infrastructure.
- **L1 Ingest Acquisition** receives edge frames, attaches or validates source identity and timing metadata, persists raw evidence, and emits source-local observations with raw references for downstream processing.
...
- **L0 Edge Capture and Transport:** emits transport frames and raw capture payloads; may not create full contract objects on constrained devices.
- **L1 Ingest Acquisition:** creates source-local `observation` objects from raw input signals or edge transport frames and persists raw evidence by reference.
...
In distributed deployments, L0 and L1 may run on different computers: L0 on edge capture hardware and L1 on a gateway or server ingest node.

## 4. Layer Definitions

### 4.1 L1 Acquisition

L1 captures physical or digital source signals without interpretation. In distributed deployments, L1 acts as ingest acquisition: it receives edge frames, persists raw payloads, and emits source observations with references.
...
Detailed schemas, required/optional status, allowed values, and serialization formats belong in D01.02 Shared Streaming Contract: `D01.02_SAR_Shared_Streaming_Contract_V1_2026-07-29.md`.

For the baseline audio ingest profile, see D01.02.01: `D01.02.01_SAR_Audio_Streaming_Profile_V1_2026-07-29.md`.

For run/session execution boundaries and continuous-activity segmentation policy, see D01.04: `D01.04_SAR_Run_and_Session_Identity_and_Boundary_Policy_V1_2026-07-28.md`.
